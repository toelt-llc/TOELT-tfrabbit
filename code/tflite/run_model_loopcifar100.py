...
cifar_models_dir = pathlib.Path('./cifar100_models')
...
## Run TFLite 
tflite_models = []
for dirname, _, filenames in os.walk('./cifar100_tflite_models/'):
...
## Run classic TF part 
cnn_model = tf.keras.models.load_model('./cifar100_models/CNN_classic.h5')
ffnn_model = tf.keras.models.load_model('./cifar100_models/FFNN_classic.h5')
...
with open('RPI_inferences_cifar_100_'+str(num_iter)+name+'.pkl', 'wb') as f:
    pickle.dump(data, f)

result.to_csv('RPI_inferences_cifar_100_'+str(num_iter)+name+'.csv', index=False)